...
class MpcSolverNoGravity(object):
    ...
    def __init__(self, wx=wx, wu=wu, dt=0.1, N=10):
        self._wx = wx
        self._wu = wu
        self._H = N
        solverName = 'mpcSolver_panda_no_gravity_real_dt' + str(dt).replace('.','') + "_N" + str(N)
        mpcFileName = os.path.dirname(os.path.abspath(__file__)) + '/codeGen/' + solverName
    ...
    def solve(self, ob):
        ...
        if exitflag < 0:
            action = np.zeros(7)
            print("WARNING MPC FAILED, PUBLISHING ZERO")
            return action, info
